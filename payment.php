...
if (!$draft) { header('Location: dashboard.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
 if (!hash_equals($_SESSION['csrf_token'],$_POST['csrf_token']??'')) $error='Your session token expired. Refresh the page and try again.';
  ...
  if ($action==='confirm_booking') {
   ...
   $pdo->commit(); unset($_SESSION['booking_draft']); header('Location: dashboard.php?booking_confirmed='.$bookingId); exit;
  }
...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Confirm booking — Nexora</title><link rel="stylesheet" href="output.css"><link rel="stylesheet" href="styles.css"><link rel="stylesheet" href="payment.css?v=4.1"></head><body class="payment-page">
<header class="payment-header"><nav class="payment-nav"><a href="index.php" class="payment-brand"><img src="Images/nexora-logo.png" alt="Nexora"></a><div class="payment-security"><span>✓</span> Secure booking step</div></nav></header>
...
<div class="payment-heading"><div><span class="payment-eyebrow">Final review</span><h1>Confirm your booking</h1><p>Select a saved payment method or add a new one before submitting your reservation.</p></div><div class="payment-status-chip">Pending admin approval after submission</div></div>
...
<aside class="payment-summary-card"><div class="payment-summary-title"><span>Booking summary</span><a href="fleet.php">Change vehicle</a></div><div class="payment-vehicle"><div class="payment-vehicle-mark">N</div><div><strong><?=e($vehicleName)?></strong><span><?=e(ucfirst($vehicle['transmission']))?> transmission</span></div></div><dl class="payment-details"><div><dt>Pick-up</dt><dd><?=e(displayDate($draft['pickup_date']))?></dd></div><div><dt>Return</dt><dd><?=e(displayDate($draft['return_date']))?></dd></div><div><dt>Duration</dt><dd><?=$totalDays?> day<?=$totalDays===1?'':'s'?></dd></div><div><dt>Daily rate</dt><dd>₱<?=number_format((float)$vehicle['daily_rate'],2)?></dd></div></dl><div class="payment-location-block"><span>Pick-up location</span><strong><?=e($draft['pickup_location'])?></strong></div><div class="payment-total"><span>Estimated total</span><strong>₱<?=number_format($totalAmount,2)?></strong></div><p class="payment-summary-note">Confirming submits the reservation. The booking remains pending until Nexora approves it.</p></aside></div><div class="payment-bottom-links"><a href="fleet.php">← Back to fleet</a><a href="contact.php">Need help? Contact support</a></div></main>
