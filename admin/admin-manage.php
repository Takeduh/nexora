...
<main class="dash-shell">
    ...
    <?php if ($section === 'bookings'): ?>
        <?php if ($editBooking): ?><section class="crud-grid one-form"><article class="dash-panel crud-form-card"><span class="dash-section-label"><?= in_array($editBooking['status'], ['completed','cancelled'], true) ? 'Read only' : 'Update' ?></span><h2>Edit booking #<?= (int)$editBooking['id'] ?></h2><?php if (in_array($editBooking['status'], ['completed','cancelled'], true)): ?><p>This booking is <?= e($editBooking['status']) ?> and can no longer be changed.</p><?php if (!empty($editBooking['cancellation_reason'])): ?><p><strong>Cancellation reason:</strong> <?= e($editBooking['cancellation_reason']) ?></p><?php endif; ?><a class="dash-secondary-btn" href="?section=bookings">Back</a><?php else: ?><form action="admin-crud.php" method="post" class="crud-form"><input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token']) ?>"><input type="hidden" name="return_to" value="<?= e($returnTo) ?>"><input type="hidden" name="action" value="booking_update"><input type="hidden" name="booking_id" value="<?= (int)$editBooking['id'] ?>"><label>Pickup location<input name="pickup_location" required value="<?= e($editBooking['pickup_location']) ?>" <?= $editBooking['status']==='active'?'readonly':'' ?>></label><div class="crud-form-row"><label>Pickup date<input type="date" name="pickup_date" required value="<?= e($editBooking['pickup_date']) ?>" <?= $editBooking['status']==='active'?'readonly':'' ?>></label><label>Return date<input type="date" name="return_date" required value="<?= e($editBooking['return_date']) ?>" <?= $editBooking['status']==='active'?'readonly':'' ?>></label></div><label>Status<select name="status"><?php foreach (bookingStatusOptions($editBooking['status']) as $s): ?><option value="<?= $s ?>" <?= selected($editBooking['status'], $s) ?>><?= ucfirst($s) ?></option><?php endforeach; ?></select><small class="booking-workflow-note">Allowed flow: pending → confirmed → active → completed. Pending/confirmed may be cancelled.</small></label><label>Cancellation reason<textarea name="cancellation_reason" maxlength="500" rows="3" placeholder="Required only when changing status to cancelled"></textarea></label><label>Special requests<textarea name="special_requests" rows="4" <?= $editBooking['status']==='active'?'readonly':'' ?>><?= e((string)$editBooking['special_requests']) ?></textarea></label><div class="crud-actions"><button class="dash-primary-btn">Save booking</button><a class="dash-secondary-btn" href="?section=bookings">Cancel</a></div></form><?php endif; ?></article></section><?php endif; ?>
        <section class="dash-panel admin-section"><div class="dash-panel-head"><div><span class="dash-section-label">Read / update</span><h2>Bookings</h2></div><span>Hard delete disabled for audit history</span></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Booking</th><th>Customer</th><th>Trip</th><th>Total</th><th>Status</th><th>Actions</th></tr></thead><tbody><?php foreach ($bookings as $row): ?><tr><td><strong>#<?= (int)$row['id'] ?> · <?= e($row['vehicle_name']) ?></strong><small><?= e(ucfirst((string)$row['transmission'])) ?></small></td><td><?= e($row['first_name'] . ' ' . $row['last_name']) ?><small><?= e($row['email']) ?></small></td><td><?= e($row['pickup_date']) ?> → <?= e($row['return_date']) ?><small><?= e($row['pickup_location']) ?></small></td><td>₱<?= number_format((float)$row['total_amount'],2) ?></td><td><?= e(ucfirst($row['status'])) ?></td><td><div class="crud-row-actions"><a class="mini-action" href="?section=bookings&edit_booking=<?= (int)$row['id'] ?>">Edit</a><?php if (in_array($row['status'], ['pending','confirmed'], true)): ?><details class="admin-cancel-details"><summary class="mini-action danger">Cancel</summary><form action="admin-crud.php" method="post" class="admin-cancel-form" onsubmit="return confirm('Cancel this booking?');"><input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token']) ?>"><input type="hidden" name="return_to" value="<?= e($returnTo) ?>"><input type="hidden" name="action" value="booking_cancel"><input type="hidden" name="booking_id" value="<?= (int)$row['id'] ?>"><textarea name="cancellation_reason" maxlength="500" required placeholder="Cancellation reason"></textarea><button class="mini-action danger">Confirm</button></form></details><?php endif; ?></div></td></tr><?php endforeach; ?></tbody></table></div></section>
    <?php endif; ?>
